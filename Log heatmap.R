...
# Replace "beta"/"alpha" with Greek letters in substrate names
annotation_row$Substrate <- gsub("beta-glucan", "β-glucan", annotation_row$Substrate)
annotation_row$Substrate <- gsub("beta-mannan", "β-mannan", annotation_row$Substrate)
annotation_row$Substrate <- gsub("beta-galactan", "β-galactan", annotation_row$Substrate)
annotation_row$Substrate <- gsub("alpha-glucan", "α-glucan", annotation_row$Substrate)

# Updated substrate_colors with Greek names
substrate_colors <- c(
  'α-glucan' = 'lightpink',
  'arabinan' = 'orchid2',
  'β-galactan' = 'olivedrab3',
  'β-glucan' = 'mediumpurple1',
  'β-mannan' = 'royalblue',
  'fructan' = 'orange',
  'pectin' = 'lightcoral',
  'starch' = 'turquoise2',
...
# Add title with custom size
#grid::grid.text("CAZyme families linked to dietary fibers identified in bacterial strains",
 #               x = 0.5, y = 0.98, gp = gpar(fontsize = 26, fontface = "bold"))

p <- pheatmap(
  t(heatmap_matrix_normalized),  # transpose the matrix
  cluster_rows = FALSE,          # previously columns
  cluster_cols = TRUE,           # previously rows
  angle_col = 90,
  fontsize = 15,
  annotation_row = annotation_row,  # was annotation_col
  annotation_colors = list(Substrate = substrate_colors),
  breaks = seq(min(heatmap_matrix_normalized), max(heatmap_matrix_normalized), length.out = 100),
  show_rownames = TRUE, 
  show_colnames = TRUE, 
  border_color = NA,
  labels_row = family_names,         # used to be labels_col
  labels_col = strain_labels_expr,   # used to be labels_row
  legend = TRUE
)
